...
namespace CLAServer.Controllers
{
    ...
    public class StudentsController : Controller
    {
        ...
        private readonly StudentViewModelValidator _studentViewModelValidator;
        ...
        public JsonResult Add(StudentViewModel studentToAdd)
        {
            try
            {
                var validationResult = _studentViewModelValidator.Validate(studentToAdd);
        ...
        public JsonResult Edit(StudentViewModel studentToEdit)
        {
            try
            {
                var validationResult = _studentViewModelValidator.Validate(studentToEdit);
                if (!validationResult.IsValid)
                {
                    return Json(new { result = validationResult.Errors.Select(x => x.ErrorMessage).ToList() }, JsonRequestBehavior.AllowGet);
                }
